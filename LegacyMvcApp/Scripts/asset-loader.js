        ...
        mutationRecords.forEach(function (mutation) {
            if (typeof mutation.addedNodes == "object") {
                mutation.addedNodes.forEach(function (node) {
                    if (node.tagName?.startsWith("FRAGMENT-")
                        && knownFragments[node.tagName] === undefined) {                       
                        onNewFragment(node.tagName.toLowerCase());
                        knownFragments[node.tagName] = true;
                    }
                });
            }
        });
...
findNewFragments((nodeName) => {
    console.log(nodeName);
    [_, appName, fragmentName] = nodeName.split("-");
    fetch(`/a/${appName}/fragments/${fragmentName}/`)
        .then(res => res.json())
		.then(manifest => {
